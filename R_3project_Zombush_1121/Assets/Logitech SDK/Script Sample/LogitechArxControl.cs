...
public class LogitechArxControl : MonoBehaviour {
	...
	{ 
		...
		{
			if(!LogitechGSDK.LogiArxAddFileAs("AppletData//applet.html","applet.html", ""))
			{
				Debug.Log("Could not send applet.html : "+LogitechGSDK.LogiArxGetLastError());
			}
			if(!LogitechGSDK.LogiArxAddFileAs("AppletData//background.png","background.png",""))
			{
			...
			byte [] gameoverImageBytes = File.ReadAllBytes("AppletData//gameover.png");
